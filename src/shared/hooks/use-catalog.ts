...
export function useUpdateMaterial() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: string
      data: Parameters<typeof updateMaterial>[1]
...
export function useSetMaterialServices() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({
      inventoryItemId,
      ...
      serviceIds: string[]
    }) => setMaterialServices(inventoryItemId, serviceIds),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: catalogKeys.all })
      toast.success('Services linked')
    },
    onError: () => {
      toast.error('Failed to link services')
    },
  })
}
